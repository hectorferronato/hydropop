...
create index if not exists hydration_events_user_bottle_occurred_at_idx
...
begin
  ...
  if p_event_type = 'event_reversed' then
    ...
    v_is_active_cycle := pg_catalog.coalesce(
...
revoke all on function public.process_hydration_event(
  uuid,
  text,
  timestamptz,
  text,
