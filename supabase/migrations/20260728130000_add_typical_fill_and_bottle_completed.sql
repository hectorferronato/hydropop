...
alter table public.bottles
add constraint bottles_typical_fill_ml_check
check (
  typical_fill_ml is null
  or (
    typical_fill_ml > 0
    and typical_fill_ml <= capacity_ml
  )
);

alter table public.hydration_events
drop constraint hydration_events_event_type_check;

alter table public.hydration_events
add constraint hydration_events_event_type_check
check (
  event_type in (
    'fill_started',
    'refill',
...
alter table public.hydration_events
drop constraint hydration_events_volume_matches_type_check;

alter table public.hydration_events
add constraint hydration_events_volume_matches_type_check
check (
...
comment on function public.process_hydration_event(
  ...
  'Atomically records normal bottle completions and corrections for auth.uid().';
